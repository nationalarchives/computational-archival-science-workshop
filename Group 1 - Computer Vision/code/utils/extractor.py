...
class Extractor(object):
    ...
    def extract_from_array(self, img_array):
        ...
        :param img_array: 4-D uint8 numpy image array, NxHxWxC
        :return: NxD feature array
        """
        if len(img_array.shape) == 3:
            img_array = img_array[None, ...]
        ims = []
        for i in range(len(img_array)):
            im = Image.fromarray(img_array[i]).resize(self.img_shape, Image.BILINEAR)
            ims.append(np.array(im, dtype=np.uint8)[None, ...])
